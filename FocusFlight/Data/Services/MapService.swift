...
/// Абстракция гео- и камерных расчётов для полёта (Dependency Inversion).
protocol MapServiceProtocol: AnyObject {
    func intermediatePoint(from: CLLocationCoordinate2D, to: CLLocationCoordinate2D, fraction: Double) -> CLLocationCoordinate2D
    func calculateGreatCirclePath(from: CLLocationCoordinate2D, to: CLLocationCoordinate2D, steps: Int) -> [CLLocationCoordinate2D]
    ...
    func calculateOverviewCameraPosition(from: CLLocationCoordinate2D, to: CLLocationCoordinate2D) -> MapCamera
    func calculateFollowingCameraPosition(planePosition: CLLocationCoordinate2D, planeHeading: Double) -> MapCamera
}
